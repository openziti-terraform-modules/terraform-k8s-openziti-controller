resource "helm_release" "ziti_controller" {
  ...
  values = [yamlencode(merge({
    ...
    defaultRouter = {
      enabled = var.default_router_enabled
    }
    clientApi = {
...
data "kubernetes_secret" "default_router_token" {
  count      = var.default_router_enabled == true ? 1 : 0
  depends_on = [helm_release.ziti_controller]
  metadata {
    name      = "${var.ziti_controller_release}-router-token"
    namespace = var.ziti_namespace
  }
}
